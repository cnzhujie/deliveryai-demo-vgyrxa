...
export function MenuView({ diners, soldOut, onAdd }: MenuViewProps) {
  ...
  return (
    <section className="min-w-0">
      <div className="relative overflow-hidden rounded-3xl bg-charcoal-900 p-5 text-white shadow-card sm:p-7 dark:shadow-card-dark">
        ...
        <div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex -space-x-2">{diners.map((name, index) => <span key={name} title={name} className={`flex h-9 w-9 items-center justify-center rounded-full border-2 border-charcoal-900 text-xs font-bold ${index === 0 ? 'bg-chili-500' : index === 1 ? 'bg-amber-400 text-charcoal-900' : 'bg-rice-200 text-charcoal-900 dark:bg-charcoal-700 dark:text-rice-100'}`}>{name.slice(0, 1)}</span>)}</div>
        </div>
      </div>
      ...
      <div className="sticky top-24 z-20 -mx-4 mt-5 bg-rice-100/95 px-4 pb-3 backdrop-blur lg:mx-0 lg:px-0 dark:bg-charcoal-900/95">
        <div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-charcoal-500 dark:text-rice-300" size={18} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t('menu.search_placeholder')} className="h-12 w-full rounded-2xl border border-charcoal-900/5 bg-white pl-11 pr-4 text-sm shadow-sm outline-none transition focus:border-chili-500/30 focus:ring-4 focus:ring-chili-50 dark:border-white/10 dark:bg-charcoal-800 dark:text-rice-100 dark:focus:ring-chili-900/30" /></div>
        <div className="scrollbar-none mt-3 flex gap-2 overflow-x-auto pb-1">
          {categories.map((item) => <button key={item} onClick={() => setCategory(item)} className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-bold transition ${category === item ? 'bg-chili-500 text-white shadow-md' : 'bg-white text-charcoal-500 hover:text-chili-500 dark:bg-charcoal-800 dark:text-rice-300 dark:hover:text-chili-400'}`}>{t(item)}</button>)}
        </div>
      </div>
      ...
      <div className="mt-3 grid gap-4 sm:grid-cols-2">
        {visible.map((product, index) => {
          ...
          return (
            <article key={product.id} className="group animate-rise overflow-hidden rounded-3xl border border-charcoal-900/5 bg-white shadow-card transition hover:-translate-y-1 dark:border-white/10 dark:bg-charcoal-800 dark:shadow-card-dark" style={{ animationDelay: `${index * 40}ms` }}>
              <div className="relative h-40 overflow-hidden">
                ...
                {unavailable && <span className="absolute inset-0 flex items-center justify-center bg-white/55 text-lg font-extrabold text-charcoal-900 backdrop-blur-sm dark:bg-charcoal-800/55 dark:text-rice-200">{t('menu.sold_out')}</span>}
                {product.orderedCount && <span className="absolute bottom-3 left-3 flex items-center gap-1 rounded-full bg-white/90 px-2 py-1 text-xs font-bold text-chili-600 backdrop-blur-sm dark:bg-charcoal-700/90 dark:text-chili-400"><Check size={13} />{t('menu.ordered_table', { count: product.orderedCount })}</span>}
              </div>
              <div className="p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-extrabold text-charcoal-900 dark:text-rice-100">{t(product.name)}</h3><p className="mt-1 line-clamp-1 text-xs text-charcoal-500 dark:text-rice-300">{t(product.description)}</p></div><Button size="icon" disabled={unavailable} onClick={() => openSpec(product)} className="h-10 w-10 shrink-0 rounded-full"><Plus size={18} /></Button></div><p className="mt-4 text-xl font-extrabold text-chili-500">{money(product.price)} <small className="text-xs font-medium text-charcoal-500 dark:text-rice-300">{t('menu.from')}</small></p></div>
            </article>
          )
        })}
      </div>
      ...
      <Dialog open={!!selected} onOpenChange={(open) => { if (!open) { setSelected(null); setShowRiskWarning(false) } }}>
        <DialogContent title={selected ? t(selected.name) : ''}>
          {selected && <>
            <div className="mt-4 flex gap-4 rounded-2xl bg-white p-3 dark:bg-charcoal-800"><img src={selected.image} alt={t(selected.name)} className="h-24 w-24 rounded-xl object-cover" /><div><p className="text-sm leading-6 text-charcoal-500 dark:text-rice-300">{t(selected.description)}</p><p className="mt-2 text-xl font-extrabold text-chili-500">{money(selected.price)} <small className="text-xs font-medium text-charcoal-500 dark:text-rice-300">{t('menu.from')}</small></p></div></div>
            ...
            <div className="mt-5"><p className="mb-3 flex items-center gap-2 text-sm font-bold text-charcoal-900 dark:text-rice-100"><Users size={16} />{t('menu.who_ordered')}</p><div className="flex gap-2">{diners.map((name) => <button key={name} onClick={() => setDiner(name)} className={`rounded-full px-4 py-2 text-sm font-bold ${diner === name ? 'bg-amber-400 text-charcoal-900' : 'bg-white text-charcoal-500 dark:bg-charcoal-800 dark:text-rice-300'}`}>{name}</button>)}</div></div>
      ...
      <Dialog open={showRiskWarning} onOpenChange={(open) => !open && cancelSuperSpicy()}>
        <DialogContent title={t('menu.super_spicy_warning_title')}>
          <div className='mt-4 flex flex-col items-center gap-4'>
            <span className='flex h-14 w-14 items-center justify-center rounded-2xl bg-chili-50 text-chili-500 dark:bg-chili-900/30'><AlertTriangle size={28} /></span>
